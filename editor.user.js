	...
	var rules = [
		{
			...
			expr: new RegExp(
				'((?:^|[^[A-Za-z0-9\\-\\.])' + SUBDOM.source + ')(' +
					...
					/(?:(?:-?(?:a|an|abc|address|another|any|app|back|bad|banks?|bar|blah?|cdn|clients?|(?:(?<=\\b)co)|company|companies|custom|domains?|email|end|ever|evil|examples?|fake|fallback|foo|front|good|guys?|hacks?|hackers?|harm|harmless|hello|hi|home|hosts?|hosters?|info|information|local|mail|main|mine|my|names?|new|of|old|other|our|pages?|place|proxy|safe|samples?|servers?|services?|sites?|shops?|some|stores?|stuff|tests?|their|thing|this|url|web|what|where|x{3,}|xyz|your))+(?:-?(?:[0-9]+|[A-Za-z]))?)\\?/.source +
				')('+TLD.source +')'+
				/(\.?(?:[\;\,\:\/_\"\*'\)\>\?\!\` \t\$]|$))/.source
			,'gmi'),
